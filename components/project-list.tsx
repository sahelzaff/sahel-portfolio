...
export function ProjectList({ items }: ProjectListProps) {
  ...
  return (
    <div className="mt-10 border-t border-white/6 sm:mt-14">
      {items.map((project, index) => {
        const open = openIndex === index;

        return (
          <div key={project.number} className="project-row border-b border-white/6">
            <button
              type="button"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="mt-2 font-mono text-[0.62rem] uppercase tracking-[0.14em] text-muted transition-colors duration-300 group-hover:text-ink/70 sm:mt-3 sm:text-[0.68rem] sm:tracking-[0.18em]">
                  {project.tags.join(" · ")}
                </div>
              </div>
            ...
            <AnimatePresence initial={false}>
              {open ? (
                ...
                >
                  <div className="grid gap-5 px-0 pb-6 pl-8 pr-0 text-[0.95rem] leading-7 text-paper/70 sm:pb-7 sm:pl-11 md:grid-cols-3 md:gap-8 md:pl-[5.5rem] md:text-[0.98rem] md:leading-8">
                    <div>
                      <div className="mb-2 font-mono text-[0.68rem] uppercase tracking-[0.2em] text-teal">Mission Brief</div>
                      <p>{project.detail.problem}</p>
                    </div>
                    <div>
                      <div className="mb-2 font-mono text-[0.68rem] uppercase tracking-[0.2em] text-teal">System Built</div>
                      <p>{project.detail.build}</p>
                    </div>
                    <div>
                      <div className="mb-2 font-mono text-[0.68rem] uppercase tracking-[0.2em] text-amber">Mission Status</div>
